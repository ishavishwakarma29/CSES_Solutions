...
pair<int, int> bfs(int node, vector<int>* gr)
{
...
void dfs(int par, int node, int d, int c, vector<int>* gr, vector<vector<int>>& )
{
    dp[node][c]=d;
    for(auto child : gr[node])
    {
        if(child!=par){
            dfs(node, child, d+1, c, gr, dp);
        }
    }
}
...
int32_t main()
{
    ...
    while(t--)
    {
        ...
        pair<int, int> a = bfs(1, gr);
        pair<int, int> b = bfs(a.first, gr);


        dp.assign(n+1, vector<int>(2, 0));

        dfs(0, a.first, 0, 0, gr, dp);
        dfs(0, b.first, 0, 1, gr, dp);
